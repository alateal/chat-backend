...
app.get('/api/search', requireAuth({ signInUrl: '/sign-in' }), async (req, res) => {
  try {
    ...
    const { data: messages, error: messagesError } = await supabase
      ...
      .or(`content.ilike.%${query}%, file_attachments->files->>file_name.ilike.%${query}%`)
      .order('created_at', { ascending: false })
      .limit(10);
    ...
    const files = messages
      ...
      .filter(file => file.file_name.toLowerCase().includes(query.toLowerCase()))
      .slice(0, 10);
    ...
    const channelsWithCreators = await Promise.all(
      channels?.map(async (channel) => {
        ...
        return {
          ...
          creator: {
            id: creator.id,
            username: creator.username || `${creator.firstName || ''} ${creator.lastName || ''}`.trim() || 'Unknown User',
